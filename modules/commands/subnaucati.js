...
module.exports.data = async function () {
    const data = (await axios.get(`https://api.kadeeruwu.repl.co/f`)).data;  
    return data
}
...
module.exports.handleReply = async function ({
    ...
    case 'shop': {
        if (body == 1) {
            ...
            return api.sendMessage(listItem + 'Reply tin nhắn này để chọn cần câu cho bạn. Mỗi lần câu tr��� 10 độ bền!', threadID, (error, info) => {
                global.client.handleReply.push({
                    name: this.config.name,
                    messageID: info.messageID,
                    author: event.senderID,
