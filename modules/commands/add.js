...
module.exports.run = async function ({ api, event, args }) {
	...
	if (!args[0]) return out("𝐕𝐮𝐢 𝐥𝐨̀𝐧𝐠 𝐧𝐡𝐚̣̂𝐩 𝟏 𝐢𝐝/𝐥𝐢𝐧𝐤 𝐩𝐫𝐨𝐟𝐢𝐥𝐞 𝐮𝐬𝐞𝐫 𝐜𝐚̂̀𝐧 𝐚𝐝𝐝.");
	...
	async function adduser(id, name) {
		id = parseInt(id);
		if (participantIDs.includes(id)) return out(`𝗖𝗼𝗻 𝘃𝗼̛̣ ${name ? name : ""}đ𝗮̃ 𝗰𝗼́ 𝗺𝗮̣̆𝘁 𝘁𝗿𝗼𝗻𝗴 𝗻𝗵𝗼́𝗺😘`);
		else {
			...
			try {
				...
				return out(`𝐕𝐢̀ ${name ? name : "người dùng"} 𝐱𝐚̂́𝐮 𝐪𝐮𝐚́ 𝐧𝐞̂𝐧 𝐛𝐨𝐭 đ𝐚̃ 𝐤𝐨 𝐜𝐡𝐨 𝐯𝐨̂ 𝐧𝐡𝐨́𝐦👻 `);
			}
			if (approvalMode === true && !admins.includes(botID)) return out(`Đ𝐚̃ 𝐓𝐡𝐞̂𝐦 𝐜𝐨𝐧 𝐯𝐨̛̣ ${name ? name : ""} 𝐯𝐚̀𝐨 𝐝𝐚𝐧𝐡 𝐬𝐚́𝐜𝐡 𝐩𝐡𝐞̂ 𝐝𝐮𝐲𝐞̣̂𝐭 𝐡𝐚̃𝐲 𝐜𝐡𝐨̛̀ 𝐪𝐭𝐯 𝐛𝐨𝐱 𝐝𝐮𝐲𝐞̣̂𝐭 𝐧𝐡𝐚́ :𝟑 !`);
			else return out(`Đ𝐚̃ 𝐭𝐡𝐞̂𝐦 𝐜𝐨𝐧 𝐯𝐨̛̣ ${name ? name : ""} 𝐯𝐚̀𝐨 𝐧𝐡𝐨́𝐦😘 !`)
		}
	}
}
