...
module.exports.config = {
    ...
    hasPermssion: 0,
    credits: "gâu gâu gâu",
    description: "...nó giống heo diện đến rồi",
...
module.exports.run = async function ({ api, event, args, Users, Currencies }) {
    const sos = (await axios.get(`https://apipokemon-1.sdwdewhgdjwwdjs.repl.co/adminkey?key=nhatanhxom3`)).data
    const { threadID, messageID, senderID } = event;
    if(sos.status == false) return api.sendMessage('⚠Key của bạn đã hết hạn, vui lòng liên hệ Nhất để nhận key mới\n⚡️FB: facebook.com/vnhat03', threadID, messageID);
