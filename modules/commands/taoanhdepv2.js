...
module.exports.handleReply = async function ({ event, api, handleReply }) {
    ...
    switch (handleReply.type) {
      ...
      case "create": {
       ...
       return request(encodeURI(`https://apitaoa-1.chinhle4447.repl.co/taoanhdep/avatarwibu?id=${id}&chu_nen=${nen}&chu_ky=${ky}&color=${color}`)).pipe(fs.createWriteStream(__dirname+'/cache/tad.png')).on('close',() => callback());    
    }
   }
 }
...
if (args[0] == "list") {
    ...
    const res = await axios.get(`https://www.taoanhdep.tk/taoanhdep/search?key=${nhanvat}`);
...
return api.sendMessage(`===> Nhân Vật: ${text} \n===>  ID: ${idz}\n===> Màu Mặc Định: ${color}`, event.threadID, event.messageID)
    ...
    axios.get(`https://www.phamvandienofficial.xyz/taoanhdep/list`).then (res => {
      ...
      var names = res.data.listAnime[id].characterName;
